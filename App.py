...
def ViewForm():
    ...
    if (len(data)==0):
        ...
        return render_template('ViewForm.html', preguntas = data, opciones = data2, valores = data3)
    else:
        return render_template('ViewForm.html', preguntas = data, opciones = data2, valores = data3)
...
def createOptions():
     return render_template('createOptions.html')

@app.route('/createValue')
def createValue():
     return render_template('createValue.html')
...
def insertQuestion():
    if request.method == 'POST':
        ...
        cur.execute('INSERT INTO preguntas(TexPreg, EstadoPreg, FecRegPreg, HoraRegPreg) VALUES (%s,%s,CURDATE(),CURTIME())',(question,estado)) ## escrbimos la consulta
        mysql.connection.commit()
        id = cur.lastrowid
        if cur.rowcount >= 1:
            flash('Pregunta agregada satisfactoriamente')
            return render_template('createOptions.html',id=id) ## nombre de la ruta
        else:
            flash('!ha ocurrido un error al ingresar las preguntas')
            #return 'Consulta false'   
            return redirect(url_for('createOptions')) ## nombre de la ruta

@app.route('/insertOption',methods=['POST'])
def insertOption():
    if request.method == 'POST':
        idpreg = request.form['id-op']
        answer = request.form['opcion']
        cur = mysql.connection.cursor() ## obtenemos conexion
        cur.execute('INSERT INTO opciones(IdPreguntas, TextOpc, FecRegOpc, HoraRegOpc) VALUES (%s,%s,CURDATE(),CURTIME())',(idpreg,answer)) ## escrbimos la consulta
        mysql.connection.commit()
        id = cur.lastrowid
        if cur.rowcount >= 1:
            flash('Respuesta agregada satisfactoriamente')
            return render_template('createValue.html',id=id) ## nombre de la ruta
        else:
            flash('!ha ocurrido un error al ingresar las preguntas')
            #return 'Consulta false'   
            return redirect(url_for('createValue')) ## nombre de la ruta

@app.route('/insertValue',methods=['POST'])
def insertValue():
    if request.method == 'POST':
        idopc = request.form['id-val']
        segment = request.form['segmento']
        value = request.form['valor']
        cur = mysql.connection.cursor() ## obtenemos conexion
        cur.execute('INSERT INTO opciones(IdOpciones,CarValor, Valor, FecRegVal, FecHorVal) VALUES (%s,%s,%s,CURDATE(),CURTIME())',(idopc,segment,value)) ## escrbimos la consulta
        mysql.connection.commit()
        return render_template('formulario.html') ## nombre de la ruta
...
def VistaFormulario():
    ...
    return render_template('formulario.html', preguntas = data, opciones = data2, valores = data3)
